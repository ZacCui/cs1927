...
int connections(Map g, LocationID start, LocationID end, TransportID type[])
{
   ...
   int numC = 0;  //counter for numConnections / type array
   
   // check if end is adjacent (or if you can travel by boat)
   VList cur;
   VList cur2;
   ...
   for (cur = g->connections[start]; cur != NULL; cur = cur->next) {
      ...
      } else if (!isSea(start) && (cur->type == BOAT) && !isSea(end)){
         // boat from land to land
      
         for (cur2 = g->connections[cur->v]; cur2 != NULL; cur2 = cur2->next) {
            if (cur2->v == end) {
               // can go from sea to intended end land
               type[numC] = cur2->type;
               numC++;
            }
         }  

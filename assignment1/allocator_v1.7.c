// Completed by ZIYI CUI (z5097491)
//
...
vsize_t Roundup2(int size);
vsize_t Roundup4(int size);
void Slipt_delete(vsize_t size, free_header_t* freep);
...
void clean(free_header_t * L);
vsize_t getNumFreeBlock(void);
free_header_t * findsamllest(free_header_t * FLP_OLD , vsize_t size);
...
void *vlad_malloc(u_int32_t n){
    ...
    vsize_t Threshold = 2*FREE_HEADER_SIZE + n + ALLOC_HEADER_SIZE;
    
    free_header_t * FLP_OLD = (free_header_t *)Create_p(free_list_ptr);
    
    FLP_OLD = findsamllest(FLP_OLD,size);
    if(FLP_OLD == NULL){
...
void vlad_stats(void)
{
    ...
    printf("The number of free blocks %d\n", getNumFreeBlock());
    showMemory();
    return;
}
...
void Slipt_delete(vsize_t size, free_header_t* freeP){
    ...
    vaddr_t position = size + getAddrIndex(freeP);
    free_header_t * freeP1 = (free_header_t *)Create_p(position);
...
void showMemory(void){
    ...
    vsize_t k = 0;
    int Judge = 0;
    unsigned int * A = (unsigned int *)memory;
    
    while(i < memory_size/4 && k < memory_size){
        if(*(A+i) == MAGIC_FREE){
            free_header_t * F =(free_header_t*)(A+i);
            printf("[\033[32;1mF\033[0m-S:%-4d P:%-4d N:%-4d]",F->size,F->prev,F->next);
            i += 4;
            k += FREE_HEADER_SIZE;
            Judge = 0;
        }else if(*(A+i) == MAGIC_ALLOC){
            alloc_header_t * AL =(alloc_header_t*)(A+i);
            printf("[\033[31;1mA\033[0m-S:%-4d]", AL->size);
            i += 2;
            k += ALLOC_HEADER_SIZE;
            Judge = 1;
        }else{
...
void clean(free_header_t * L){
    assert(L->magic == MAGIC_FREE);
    byte * a = (byte*)L;
    vsize_t i = FREE_HEADER_SIZE;
    for(;i<getFreeSize(L); i++){
        a[i]= 0;
    }
}
...
vsize_t getNumFreeBlock(void){
    vsize_t num = 1;
    free_header_t* L = (free_header_t*)Create_p(free_list_ptr);
    while(L->next != free_list_ptr){
        L = (free_header_t*)Create_p(L->next);
        num++;
    }
    return num;
}
...
free_header_t * findsamllest(free_header_t * FLP_OLD , vsize_t size){
    ...
    vsize_t dummy = getFreeSize(FLP_OLD);
    free_header_t * curr = FLP_OLD;

    while(dummy != 0){
    	if(getFreeSize(curr) < dummy && getFreeSize(curr) >= size){
    		FLP_OLD = curr;
    		dummy = getFreeSize(curr);
    	}
    	curr = FindtheNext(curr);
    	if(getAddrIndex(curr) == free_list_ptr ){break;}
    }

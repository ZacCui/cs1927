...
// Written by John Shepherd, July 28

// Completed by Zac Cui(Ziyi CUI) z5097491, Huijia Wang z5097494
// Date 26/7/2016
#include <stdlib.h>
#include <stdio.h>
#include "assert.h"
...
void IntListInsertInOrder(IntList L, int v)
{
	...
	if(L->first != NULL){
		...
		for (curr = L->first; (curr != NULL && v > curr->data); curr=curr->next) {
			if(v > curr->data){
				prev = curr;
			}
		}
		
		if (curr == L->first){              // v is the minimum value
			node->next = curr;
			L->first = node;
		}else if(curr == NULL){				// v is the maximum value
			L->last->next = node;
			L->last = L->last->next;
		}else{ 				   				// v is between the first and the last	
			struct IntListNode * dummy = prev->next;
			prev->next = node;
			node->next = dummy;
		}
	}else{

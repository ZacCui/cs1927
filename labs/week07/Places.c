...
int nameToID(char *name)
{
   ...
   while(strcmp(name,places[first+last/2].name) == 0){ 
		if (strcmp(name,places[first+last/2].name) > 0) last = first+last/2;
		else first = first+last/2;
   }
   if(strcmp(name,places[first+last/2].name) == 0) {
   	return places[first+last/2].id;
   	}else{
   		return NOWHERE;
   	}

...
void DLListBefore(DLList L, char *it)
{
	...
	if( L->curr != L->first) {L->curr->prev->next = insert;}
	if( L->curr == L->first) {L->first = insert;}
	L->curr->prev = insert;
	L->curr = insert;
	L->nitems++;
}
...
void DLListDelete(DLList L)
{
	...
	if(L->first == L->last){
		...
		if(L->curr != L->first) {L->curr->prev->next = L->curr->next;}
		L->curr->next->prev = L->curr->prev;
		DLListNode * dummy = L->curr;
		if(dummy == L->first)  {L->first = dummy->next;}
		L->curr = L->curr->next;
		dummy = NULL;
	}
	L->nitems--;
